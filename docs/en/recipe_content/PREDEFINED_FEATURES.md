...
- `enableModuleCreationParams` - adds two text fields for new module's name and its main package name.
  ...
  and [`globals`](GLOBALS.md), sections and also in FTL templates you can use the following parameters:
    * `__moduleName` - module Gradle path without a leading `:`, for example `applicant:core`; during file
      generation path segments become nested directories
    * `__packageName` - the packageName of the module that is added to `AndroidManifest.xml`; when suggested from
      `__moduleName`, `:` characters are replaced with `.`
